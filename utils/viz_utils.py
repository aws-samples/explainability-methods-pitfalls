...
def plot_boundary_with_ce(
    ...
    *,
    ...
    """
    Generate a 2D plot of the model boundary, also showing the original input
    and CEs.

    :param instance: The original input.
    :param ces: Counterfactuals
    :param model: The ML models for which we are generating CEs. Should implement a predict method.
    :param x_train: The training data
    :param features_to_vary: Which features are being changed. Should be exactly 2.
    :param categorical_features: Which features are categorical
    :param grid_width: The number of buckets on the x and y axis of the 2D grid.
    :return:
    """

    assert (
        len(features_to_vary) == 2
    ), "For a 2D plot, we should vary exactly 2 features when generating counterfactuals."
    assert len(instance) == 1, "Expecting a single input for visualization"

    # Copy the input since we are going to make changes to it
    instance = instance.copy()
    ces = ces.copy()

    # Figure out the model prediction on the original and counterfactual inputs
    pred_class = model.predict(instance).item()
    print("Predicted class:", pred_class)
    ce_class = np.unique(model.predict(ces)).item()
    print("Counterfactual class:", ce_class)
    assert np.array_equal(sorted([pred_class, ce_class]), [0, 1]), (
        "Expected binary classes and original model ouput and counterfactual output to be different."
        f"Got original input = {pred_class}, model ouput = {ce_class}"
    )

    # Select the range for the 2D plot and generate the grid
    feature_to_range = {}
    for feature in features_to_vary:
        if feature in categorical_features:
            feature_to_range[feature] = np.unique(x_train[feature])
        else:
            feature_to_range[feature] = np.linspace(
                start=min(
                    x_train[feature].min(), ces[feature].to_numpy().astype(float).min()
                ),
                stop=max(
                    x_train[feature].max(), ces[feature].to_numpy().astype(float).max()
                ),
                num=grid_width,
            )
    grid = np.meshgrid(
        *[feature_to_range[feature] for feature in features_to_vary], indexing="xy"
    )
    # Generate the model output on this grid
    query_instance_grid = np.broadcast_to(
        instance.to_numpy(), (grid[0].shape[0] * grid[0].shape[1], instance.shape[1])
    )
    query_instance_grid = pd.DataFrame(query_instance_grid, columns=x_train.columns)

    for idx, feat in enumerate(features_to_vary):
        query_instance_grid[feat] = grid[idx].flatten()

    grid_out = model.predict(query_instance_grid).reshape(
        grid[0].shape[0], grid[0].shape[1]
    )

    mesh_cmap = LinearSegmentedColormap.from_list("", list(color_mapping.values()))

    # For plotting, we need the axis values to be numerical.
    # Map categorical features onto (0, 1, ..., num_categories)
    feature_to_replacements = {}
    for feature_idx, feature in enumerate(features_to_vary):
        if feature not in categorical_features:
            continue
        feature_to_replacements[feature] = {
            feature_val: idx_val
            for idx_val, feature_val in enumerate(feature_to_range[feature])
        }

        # Change the feature in the input
        instance[feature].replace(feature_to_replacements[feature], inplace=True)

        # Change the feature in the counterfactuals
        ces[feature].replace(feature_to_replacements[feature], inplace=True)

        # Change the feature in the grid
        feature_arr = grid[feature_idx]
        feature_arr_new = np.zeros(feature_arr.shape, dtype=int)
        for k, v in feature_to_replacements[feature].items():
            feature_arr_new[feature_arr == k] = v
            grid[feature_idx] = feature_arr_new

    # Generate the plot
    fig, ax = plt.subplots()
    ax.pcolormesh(*grid, grid_out, cmap=mesh_cmap, alpha=0.2)

    class_to_color_mapping = color_mapping
    ce_features = [ces[feat].to_numpy() for feat in features_to_vary]
    ax.scatter(
        *ce_features, color=class_to_color_mapping[ce_class], label="Counterfactual"
    )
    ax.scatter(
        *[instance[feat] for feat in features_to_vary],
        color=class_to_color_mapping[pred_class],
        label="Original Input",
    )

    ax.set_xlabel(features_to_vary[0])
    ax.set_ylabel(features_to_vary[1])

    fig.canvas.draw()

    # In case we have a categorical feature, put the values on the ticks
    tick_funcs = [plt.xticks, plt.yticks]

    for feature_idx, feature in enumerate(features_to_vary):
        if feature in categorical_features:
            tick_funcs[feature_idx](
                range(len(feature_to_range[feature])),
                feature_to_range[feature],
                rotation=45,
            )
...
"""
    Code for visualize_image_attr and visualize_image_attr_multiple from https://github.com/pytorch/captum/blob/master/captum/attr/_utils/visualization.py
    Imports modified and simplified.
    Updated keyword argument from b to visible for compatibility with matplotlib:  plt_axis.grid(b=False) -> plt_axis.grid(visible=False).
"""
